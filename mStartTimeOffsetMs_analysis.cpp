...
kKeyTime:
	...
	startMPEG4Recording()
		...
		setupMPEG4MetaData(startTimeUs, totalBitRate, &meta) 
				(*meta)->setInt64(kKeyTime, startTimeUs);// startTimeUs 保存在 kKeyTime 字段中，用metadata进行传递
...
下面这段代码
		if (mStszTableEntries->count() == 0) {
            ...
            mOwner->setStartTimestampUs(mStartTimestampUs); //mStartTimestampUs 是 timestampUs
            previousPausedDurationUs = mStartTimestampUs;
#ifndef ANDROID_DEFAULT_CODE
			ALOGD("%s mStartTimestampUs=%lldus", mIsAudio?"Audio":"Video", mStartTimestampUs);
			if (!mIsAudio) {
				mOwner->setVideoStartTimeUs(mStartTimestampUs); //mVideoStartTimeUs 是 timestampUs
				mOwner->notify(MEDIA_RECORDER_EVENT_INFO, MEDIA_RECORDER_INFO_START_TIMER, 0); //以收到video第一个sample作为录像的开始
			}
#endif
        }
	
======================			mTrackStartTimeOffsetUs =============
mTrackStartTimeOffsetUs只有一行赋值代码，这个世界语系统时间无关，只与帧率有关。
